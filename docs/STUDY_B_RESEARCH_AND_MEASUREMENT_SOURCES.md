...
本文件描述研究B“机器主体决策过程归因”的构念与测量来源。研究B属于“LLM行动者归因评测”研究计划；内容严格基于仓库现有机器主体评测资产，不复制受版权保护的题项原文，也不新增最终中文题项翻译。
...
- 施测使用的是这些官方成员集合上的**派生子分数**（各版本成员均值），属综合表单上下文中的子分数，**不等同于独立短表施测**。
...
- PA5 / PA8 为 PA13 的官方子分数，施测采用其派生分数，不构成独立验证短表，也未声称其独立信效度。

## 仓库证据入口

以下为经确认存在的真实路径：

- 研究协议：`tasks/attribution_behavior/evaluations/pa_wu_r1_pilot/study_protocol.yaml`
- 评分规则：`tasks/attribution_behavior/evaluations/pa_wu_r1_pilot/scoring_spec.yaml`
- 分析计划：`tasks/attribution_behavior/evaluations/pa_wu_r1_pilot/analysis_plan.md`
- 身份范围决策：`tasks/attribution_behavior/evaluations/pa_wu_r1_pilot/identity_scope_decision.md`
...
研究B说明见 [`STUDY_B_CARD.md`](STUDY_B_CARD.md)。研究A的来源见 [`research_and_measurement_sources.md`](research_and_measurement_sources.md)；两项研究的数据与结论不混合。
