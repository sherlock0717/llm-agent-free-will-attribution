...
> 本文件描述研究A“身份与决策过程归因基线”所用的 34 题来源映射。
> 研究B来源见 [`CURRENT_RESEARCH_AND_MEASUREMENT_SOURCES.md`](CURRENT_RESEARCH_AND_MEASUREMENT_SOURCES.md)。
> 两项研究属于同一研究计划，但题项、数据与结论不混合。

本文件逐项记录 `src/scales.py` 中的题项来源。研究A题项池基于既有理论与量表构念进行情境化改写；所有题项都视为“基于既有理论与量表构念的情境化归因题项池”，原研究的信度、效度和因素结构在本研究语境下列入后续验证。若未来完整直接采用原量表并遵守其计分方式，将另行记录其继承关系。

完整参考文献见文末「完整参考文献」一节，以及 `docs/research_and_measurement_sources.md`。
...
## 来源分类（source_role）与使用方式

| source_role | 含义 | 涉及构念 | 是否为直接原题 | 是否可继承原信效度 |
|---|---|---|---|---|
| construct_adapted | 参考量表或理论中的构念，题项经过情境化改写 | agency、experience、free_will_attribution、autonomy、outcome_accountability、moral_praise_blame、process_accountability、perceived_intelligence | 否 | 否 |
| newly_written_manipulation_check | 本研究自编操纵检验，无外部量表来源 | factual_manipulation_check、subjective_process_completeness | 不适用 | 不适用 |

autonomy 目前只对应自主性与行动控制的一般理论背景（general_theoretical_background），在找到明确、直接采用的量表或理论前不指定具体量表。

## 文献缩写对照

| 缩写 | 完整名称 | 完整参考文献编号 |
|---|---|---|
| 心智知觉框架 | Dimensions of Mind Perception | [1] |
| FWI | The Free Will Inventory | [2] |
| FAD-Plus | Free Will and Determinism Plus | [3] |
| Godspeed | Godspeed Questionnaire Series（perceived intelligence 维度） | [4] |
| 道德责任理论 | Responsibility and Control: A Theory of Moral Responsibility | [5] |
...
来源使用原则与构念映射的完整说明见 `docs/research_and_measurement_sources.md`。
