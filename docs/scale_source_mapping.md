...
> 本文件描述 PA—Wu R1 建立之前的 AI 与人类身份比较路线所用的 34 题来源映射，
> 不代表当前主研究的构念与题项来源。
> 当前 R1 来源请见 [`CURRENT_RESEARCH_AND_MEASUREMENT_SOURCES.md`](CURRENT_RESEARCH_AND_MEASUREMENT_SOURCES.md)。

本文件逐项记录 `src/scales.py` 中的题项来源。早期题项池基于既有理论与量表构念进行情境化改写，并非对原量表的完整直接使用；除非未来完整直接采用原量表并遵守其计分方式，否则所有题项都视为“基于既有理论与量表构念的情境化归因题项池”，原研究的信度、效度和因素结构不能直接继承。
